// =========================================================================
// 대림대학교 공식 사이트 링크 모음
// -------------------------------------------------------------------------
// 실제 대림대학교 공식 홈페이지/포털 주소입니다. 세부 페이지(특정 공지사항 등)는
// 학교 홈페이지 구조가 바뀔 수 있으니 발표 전에 링크가 살아있는지 한 번씩 클릭해서
// 확인해두세요.
// =========================================================================

export const OFFICIAL_LINKS = {
  // 대표 홈페이지
  home: "https://www.daelim.ac.kr/index.do",
  // 대림대 포털 (학사정보시스템 - 수강신청/성적조회/등록금 확인 등이 실제로 이루어지는 곳)
  portal: "https://pt.daelim.ac.kr/",
  // 장학/대출 안내 페이지 (교내장학 종류별 선정기준·지급액 표 + 교외장학 + 국가장학금·학자금대출 절차)
  scholarship: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=1010",
  // 학사일정 안내 페이지
  academicCalendar: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=930",
  // 주요사이트(학생지원) 안내 페이지 - 각종 문의처가 정리되어 있음
  studentSupportSites: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=1080",
  // 국가장학금은 대림대가 아닌 한국장학재단에서 신청
  kosaf: "https://www.kosaf.go.kr",
  // 휴학 기본사항 안내 페이지 (휴학종류/신청방법 및 승인절차/구비서류)
  leaveOfAbsenceInfo: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=780",
  // 학과별 연락처 안내 페이지
  departmentOffices: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=310&CONTENTS_NO=2",
  // 부서별 연락처 안내 페이지
  officeContacts: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=310&CONTENTS_NO=1",
  // 소단위전공 - 학과형
  minorProgramGeneral: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=5300",
  // 소단위전공 - 혁신융합대학사업
  minorProgramInnovation: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=5310#title1",
  // 소단위전공 - 신산업특화선도전문대학사업
  minorProgramIndustry: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=5310#title2",
  // 소단위전공 - 첨단산업인재양성부트캠프사업
  minorProgramBootcamp: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=5310#title3",
  // 등록금납부확인서 조회 페이지
  tuitionPaymentConfirm: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=3890",
  // 2026학년도 2학기 국가장학금 신청일정 공지
  scholarshipNationalNotice:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=990&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=9&BBS_SEQ=128818&P_BBS_SEQ=&pageNo=1",
  // 2026년도 2학기 2차 국가근로장학금 신청 공지
  scholarshipWorkStudyNotice:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=990&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=9&BBS_SEQ=128813&P_BBS_SEQ=&pageNo=1",
  // 장학금 우선감면 안내 공지
  scholarshipPriorityNotice:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=990&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=9&BBS_SEQ=128811&P_BBS_SEQ=&pageNo=1",
  // 2026학년도 2학기 2차 주거안정장학금 신청 공지 (신청방법 붙임파일 포함)
  scholarshipHousingNotice:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=990&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=9&BBS_SEQ=128819&P_BBS_SEQ=&pageNo=1",
  // 교외장학금 안내 공지 ①
  scholarshipExternal1:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=990&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=9&BBS_SEQ=128822&P_BBS_SEQ=&pageNo=1",
  // 교외장학금 안내 공지 ②
  scholarshipExternal2:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=990&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=9&BBS_SEQ=128815&P_BBS_SEQ=&pageNo=1",
  // 교외장학금 안내 공지 ③
  scholarshipExternal3:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=990&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=9&BBS_SEQ=128814&P_BBS_SEQ=&pageNo=1",
  // 성적평가 방법 및 성적산출(백분위 환산) 안내 페이지
  gradesEvaluationNotice: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=810&CONTENTS_NO=1",
  // 2026학년도 2학기 군 학점인정(군 복무경험/군 교육훈련/사회복무요원) 안내 공지
  militaryCreditNotice:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=900&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=8&BBS_SEQ=126338&P_BBS_SEQ=&pageNo=1",
  // 2026학년도 2학기 복학신청 안내 공지
  returnToSchoolNotice:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=900&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=8&BBS_SEQ=126329&P_BBS_SEQ=&pageNo=1",
  // 2026학년도 2학기 전과 신청 안내 공지
  majorTransferNotice:
    "https://www.daelim.ac.kr/cms/FrBoardCon/BoardView.do?MENU_ID=900&CONTENTS_NO=&SITE_NO=2&BOARD_SEQ=8&BBS_SEQ=126330&P_BBS_SEQ=&pageNo=1",
  // 통학버스(안양역/범계역) 안내 페이지
  shuttleBusNotice: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=460",
  // 증명서발급 안내 페이지 (증명서 종류/발급절차)
  certificateNotice: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=3870&CONTENTS_NO=1",
  // 인터넷증명발급센터 (회원가입 필요)
  certificateIssueCenter: "https://daelim.certpia.com/",
  // 계절학기 개설 안내 페이지
  seasonalSemesterNotice: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=890&CONTENTS_NO=1",
  // 학교 공식 Q&A 게시판 (로그인 필요 — 정말 다른 방법이 없을 때만 마지막 수단으로 안내)
  qnaBoard: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=1040#page1",
  // 학부/학과 안내 페이지 (전체 학과 목록·소개) - 특정 학과/전공 세부 질문에 안내
  departmentDirectory: "https://www.daelim.ac.kr/cms/FrCon/index.do?MENU_ID=1760",
};